...
async def get_models(request: Request):
    ...
    # Optional: append extra models via env (comma-separated), useful for quick expose of
    # experimental models that are known to work but may not appear in ListAvailableModels.
    # Example: EXTRA_MODELS=claude-sonnet-4.6,claude-opus-4.6
    extra_models_env = os.getenv("EXTRA_MODELS", "").strip()
    extra_models = []
    if extra_models_env:
        extra_models = [m.strip() for m in extra_models_env.split(",") if m.strip()]

    model_ids = sorted(set(list(available_model_ids) + extra_models))
